...
> **向后兼容说明**:
> - 旧版环境变量 `LANGCHAIN_API_KEY` 和 `LANGCHAIN_TRACING_V2` 仍然支持，但建议迁移到新命名
...
1. **必需变量**（LangSmith 1.0+）：
   - `LANGSMITH_API_KEY`：从 [https://smith.langchain.com](https://smith.langchain.com) 获取，格式为 `lsv2_...`
   - `LANGSMITH_TRACING`：必须设为 `"true"` 启用追踪
...
- [ ] **就绪检查**：实现`/ready`端点检查依赖服务
- [ ] **存活检查**：实现`/health`端点检查应用状态
...
- [ ] **启用追踪**：设置`LANGCHAIN_TRACING_V2=true`
- [ ] **项目配置**：为不同环境配置不同项目
