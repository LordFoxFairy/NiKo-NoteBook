...
> **版本要求**: LangChain 1.0+、LangGraph 1.0+、Python 3.10+
...
- **StateGraph**: LangGraph的状态图，用于编排复杂流程 → 详见**第三篇第7章**
...
> 💡 **提示**: 更多生产级错误处理、监控告警、故障恢复策略，详见 **第五篇《监控评估》** 和 **第八篇《生产实践》**。
...
```python
from langchain.agents import create_agent导入路径
from langchain_openai import ChatOpenAI
from langchain_core.tools import tool
...
> 💡 **深入学习**：Middleware 的完整用法、Hook 体系、自定义策略等内容，请参见 **第四篇《Middleware 工程化》**。
