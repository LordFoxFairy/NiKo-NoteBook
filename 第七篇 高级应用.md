...
# ========== 正确方式 1：使用 session 方法 ==========
async def setup_mcp_with_session():
    """使用 session 方法连接 MCP 服务器（正确方式）"""
    ...
    # MultiServerMCPClient 使用 session 方法，而不是 add_server
...
async def create_agent_with_mcp():
    ...
    # 获取工具（需要在 session 中）
    async with mcp_client.session("server", stdio_params) as session:
        tools = mcp_client.get_tools()
    ...
    agent = create_react_agent(
        llm,
        tools,
        messages_modifier="你是一个有用的助手，可以使用工具帮助用户。"
    )
