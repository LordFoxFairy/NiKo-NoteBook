...
Self-Consistency 本质上是一种**降噪 (Denoising)** 机制。

假设模型单次推理的错误率是 $\epsilon$。关键洞察是：
- **正确答案是确定的**：如果模型真的理解了问题，无论用什么推理路径，最终都会收敛到同一个答案。
- **错误答案是发散的**：如果模型不确定，不同的采样会产生各种不同的错误答案（幻觉是随机的）。

**统计原理**：设正确答案为 $a^*$，采样 $N$ 次。
- 如果模型对该问题的"理解概率"为 $p > 0.5$，那么正确答案出现次数期望为 $Np$。
- 错误答案被分散到各种不同的错误中，单个错误答案的期望出现次数 $< Np/(M-1)$，其中 $M$ 是可能答案空间大小。

**二项分布视角**：
正确答案被选中的概率为：
$$
P(\text{majority vote correct}) = \sum_{k > N/2}^{N} \binom{N}{k} p^k (1-p)^{N-k}
$$

当 $N$ 增大时，这个概率会快速趋近于 1（大数定律）。
...
CoT-SC 是**提分性价比最高**的方法，因为它不需要额外训练，只需推理时多采样。
