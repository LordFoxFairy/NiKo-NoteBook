...
#### 方法一：使用 Spark MLlib 高层 API（推荐用于快速原型）
...
#### 方法二：手动实现 MinHash LSH Band 分桶（工业级硬核实现）

这是**底层实现**，展示 Band & Row 分桶的核心逻辑，适合需要自定义调优的场景。
...
import hashlib
import mmh3  # MurmurHash3，更快的哈希函数

# ============ 配置参数 ============
NUM_HASHES = 128  # MinHash 签名长度
NUM_BANDS = 16    # LSH Band 数量
ROWS_PER_BAND = NUM_HASHES // NUM_BANDS  # 每个 Band 的行数 (128/16=8)
JACCARD_THRESHOLD = 0.8  # 相似度阈值

# ============ 初始化 Spark ============
spark = SparkSession.builder \
    .appName("MinHashLSH_Manual") \
...
df = spark.read.parquet("s3://my-bucket/pretrain-data/")
df = df.withColumn("shingles", shingle_udf(col("text")))

# ============ Step 2: 计算 MinHash 签名 ============
def compute_minhash_signature(shingles, num_hashes=NUM_HASHES):
    """
    对每个 shingle 集合计算 MinHash 签名
    返回: [h1, h2, ..., h_K]  K=num_hashes
    """
    if not shingles:
        return [2**31 - 1] * num_hashes  # 空文档返回最大值

    signature = [2**31 - 1] * num_hashes

    for shingle in shingles:
        # 对每个 hash 函数 (用不同的 seed)
        for i in range(num_hashes):
            hash_val = mmh3.hash(shingle, seed=i) & 0x7FFFFFFF  # 确保非负
            signature[i] = min(signature[i], hash_val)

    return signature

minhash_udf = udf(compute_minhash_signature, ArrayType(IntegerType()))
df = df.withColumn("minhash", minhash_udf(col("shingles")))

# ============ Step 3: LSH 分桶 (Band Hashing) ============
def generate_lsh_buckets(minhash_signature, num_bands=NUM_BANDS):
    """
    将 MinHash 签名分为 B 个 Band，每个 Band 哈希到一个桶
    返回: [(band_id, bucket_hash), ...]
    """
    if not minhash_signature:
        return []

    rows_per_band = len(minhash_signature) // num_bands
    buckets = []

    for band_id in range(num_bands):
        start_idx = band_id * rows_per_band
        end_idx = start_idx + rows_per_band
        band_values = minhash_signature[start_idx:end_idx]

        # 将这个 Band 哈希到一个桶
        band_str = ",".join(map(str, band_values))
        bucket_hash = hashlib.md5(band_str.encode()).hexdigest()[:16]  # 取前16位
...
bucket_schema = ArrayType(StructType([
    ...
    StructField("bucket_hash", StringType(), False)
]))

lsh_bucket_udf = udf(generate_lsh_buckets, bucket_schema)
df = df.withColumn("lsh_buckets", lsh_bucket_udf(col("minhash")))

# ============ Step 4: 爆炸 & Group By (找候选对) ============
# 将每个文档的桶展开
df_buckets = df.select("id", "text", "minhash", explode("lsh_buckets").alias("bucket_info")) \
    .select("id", "minhash", col("bucket_info.band_id").alias("band_id"),
            col("bucket_info.bucket_hash").alias("bucket"))

# 对每个桶，收集所有文档ID
candidates = df_buckets.groupBy("band_id", "bucket") \
    .agg(collect_set("id").alias("doc_ids"))

# 只保留有碰撞的桶（2个以上文档）
candidates = candidates.filter("size(doc_ids) > 1")
...
def generate_pairs(doc_ids):
    ...
    if len(doc_ids) > 100:
        return []
    return list(combinations(sorted(doc_ids), 2))

pair_schema = ArrayType(StructType([
    StructField("doc_a", StringType(), False),
    StructField("doc_b", StringType(), False)
]))

pair_udf = udf(generate_pairs, pair_schema)
candidate_pairs = candidates.withColumn("pairs", pair_udf(col("doc_ids"))) \
    .select(explode("pairs").alias("pair")) \
    .select(col("pair.doc_a").alias("doc_a"), col("pair.doc_b").alias("doc_b")) \
    .distinct()

# ============ Step 6: 计算真实 Jaccard 相似度（精确验证）============
def jaccard_similarity(sig1, sig2):
    """基于 MinHash 签名估计 Jaccard 相似度"""
...
# Join 回原数据获取 MinHash 签名
df_a = df.select(col("id").alias("doc_a"), col("minhash").alias("minhash_a"))
df_b = df.select(col("id").alias("doc_b"), col("minhash").alias("minhash_b"))

candidate_pairs = candidate_pairs.join(df_a, "doc_a").join(df_b, "doc_b")
candidate_pairs = candidate_pairs.withColumn(
    "jaccard",
    jaccard_udf(col("minhash_a"), col("minhash_b"))
)

# 过滤低于阈值的候选对
similar_pairs = candidate_pairs.filter(col("jaccard") >= JACCARD_THRESHOLD)

# ============ Step 7: 去重 (保留 ID 小的文档) ============
duplicates = similar_pairs.select(col("doc_b").alias("duplicate_id")).distinct()

# 反向 Join：保留不在重复列表中的文档
final_df = df.join(duplicates, df.id == duplicates.duplicate_id, "left_anti")

# ============ Step 8: 保存 ============
final_df.select("id", "text").write \
    .mode("overwrite") \
    .parquet("s3://my-bucket/deduped-data/")

print(f"原始文档数: {df.count()}")
print(f"去重后文档数: {final_df.count()}")
print(f"去除重复: {df.count() - final_df.count()}")
```
...
假设两个文档的真实 Jaccard 相似度为 $s$，则：
- 单个 Band 中所有行都相同的概率 = $s^r$ （r = rows per band）
- 至少一个 Band 匹配的概率（被识别为候选对）：
  $$
...
**调优目标**：
- **高召回（Recall）**：希望相似度 > 0.8 的文档尽可能被识别
  - 策略：增加 Band 数量（b ↑），减少 rows per band（r ↓）
  - 例如：b=32, r=4

- **高精度（Precision）**：希望候选对大部分都是真正相似的
  - 策略：减少 Band 数量（b ↓），增加 rows per band（r ↑）
  - 例如：b=8, r=16

**常见配置**：
| 场景 | NumHashes (K) | NumBands (b) | Rows (r) | 适用于 |
|-----|--------------|-------------|---------|--------|
| **严格去重** | 128 | 8 | 16 | 新闻去重（容忍度低）|
| **平衡配置** | 128 | 16 | 8 | 通用预训练数据 |
| **宽松去重** | 128 | 32 | 4 | 需要高召回率场景 |

**工程优化 Tips**：
- **Shuffle 优化**：调整 `spark.sql.shuffle.partitions` 为 2000-5000（根据数据量）
- **Connected Components**：如果 A≈B，B≈C，则 A,B,C 都应被去重。可以使用 Spark GraphX 的 `connectedComponents()` 实现
- **增量去重**：对新数据，只需与历史数据的 MinHash 签名比较，无需重新计算历史数据
