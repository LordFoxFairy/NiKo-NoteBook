...
- [三、Scaling Law：规模的力量](#三scaling-law规模的力量)
  ...
  - [3.4 2025年视角：Scaling Law的新发现](#34-2025年视角scaling-law的新发现)
...
不同数据源质量不同，直接混合会导致低质量数据"淹没"高质量数据。2025年的主流策略是**精细化的动态配比 (Dynamic Data Mixture)**。

**Llama 3 的数据配比策略（案例分析）**：
Llama 3 在训练最后阶段对数据配比进行了重大调整，这一策略被称为**退火阶段 (Annealing)** 的数据强化。

| 阶段 | 策略 | 目的 |
|------|------|------|
| **基础阶段 (First 13T tokens)** | 多样化混合 | 获取广泛的世界知识，不经过度筛选 |
| **退火阶段 (Last 2T tokens)** | **高质量上采样** | 使用高质量代码、数学、推理数据进行**数十倍上采样 (Upsampling)** |
| **效果** | 知识广度+推理深度 | 既知道"谁是美国总统"，也能解复杂的奥数题 |
...
C \approx 6 N D
$$
(注：每个token前向传播约2N，后向传播约4N，合计6N FLOPs)
...
0.2│ ╲╲╲╲╲╲╲╲╲╲╲╲╲╲╲╲╲                         ●●●●●●●
  │              ●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●
0.0│               Training Loss (早早收敛)
  └─────┬──────────────┬──────────────┬──────────────┬────────▶
      1000          5000           10000          50000      Steps
       ▲                              ▲
    记忆阶段                        顿悟时刻
...
在 Stable 阶段，模型快速吸收大量知识，但处于"高能态"（High Energy State），泛化能力尚未最优。
在 Annealing 阶段，虽然 LR 快速下降，但我们通常会**切换到更高质量的精子数据（如代码、数学题、教科书）**。这就像考试前的"重点复习"，让模型在参数收敛的同时，强化最核心的逻辑能力。
...
1. **数据**：
   - 规模从GB到TB，质量胜于数量
   - 多层清洗：基础过滤、语言检测、精确/模糊去重
   - 数据配比与课程学习

2. **预训练目标**：
   - CLM（GPT）：100%数据利用率，无预训练-微调Gap
   - MLM（BERT）：双向理解，但只有15%数据参与训练
   - Span Corruption（T5）：结合两者优势

3. **Scaling Law**：
   - Kaplan Law：模型越大越好
   - Chinchilla Law：数据量应为参数量的20倍
   - **2025新发现**：数据质量非线性价值 + 推理时计算的新维度

4. **工程挑战**：
   - 稳定性：梯度裁剪、梯度累积、学习率调度
   - 混合精度：FP16需要损失缩放，BF16更稳定，FP8是未来
   - 分布式：4D并行（DP+TP+PP+CP）成为主流
   - 内存优化：梯度检查点节省O(N) → O(√N)显存
