...
```mermaid
graph TD
    Seed[种子指令库<br>175条人工撰写] -->|1. 采样| Prompt[构造生成Prompt]
    Prompt -->|2. 生成| LLM[GPT-4/Claude-3]
    LLM -->|3. 输出| NewIns[新指令 & 实例]
    NewIns -->|4. 过滤| Filter{质量过滤<br>ROUGE相似度<0.7}
    Filter -->|通过| Pool[指令池]
    Filter -->|失败| Discard[丢弃]
    Pool -->|循环| Seed
```
