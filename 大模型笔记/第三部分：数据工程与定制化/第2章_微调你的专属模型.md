# 第2章：微调你的专属模型 (Fine-Tuning)

> "The finest steel is forged in the hottest fire." - 精钢须经烈火锻造
>
> 预训练赋予模型通用能力,微调则让它成为你的专属专家。从LoRA到DoRA,从4-bit量化到模型合并,掌握参数高效微调(PEFT)的全套武器库。
...
- 预计阅读时间: 90分钟
- 实战代码: 5个完整示例
- 前置知识: Transformer架构、反向传播、优化器
- 应用场景: 定制聊天机器人、领域适配、多任务学习
...
- [一、什么是微调?](#一什么是微调)
  - [1. 微调的本质](#1-微调的本质)
  - [2. 监督式微调(SFT)](#2-监督式微调supervised-fine-tuning-sft)
  - [3. 微调与预训练的关系](#3-微调与预训练的关系)
- [二、全量微调 vs. 参数高效微调(PEFT)](#二全量微调-vs-参数高效微调peft)
  - [1. 全量微调的挑战](#1-全量微调的挑战)
  - [2. 参数高效微调(PEFT)方法论](#2-参数高效微调peft方法论)
  - [3. LoRA: 低秩适应](#3-lora低秩适应low-rank-adaptation)
    - [3.1 核心原理: 低秩分解](#31-核心原理低秩分解)
    - [3.2 为什么低秩假设成立?](#32-为什么低秩假设成立)
    - [3.3 LoRA实现细节](#33-lora实现细节)
    - [3.4 LoRA应用策略](#34-lora应用策略)
  - [4. LoRA家族演进](#4-lora家族演进)
    - [4.1 QLoRA: 量化+LoRA](#41-qlora量化--lora)
    - [4.2 DoRA: 权重分解的LoRA](#42-dora权重分解的lora)
    - [4.3 AdaLoRA: 自适应秩分配](#43-adalora自适应秩分配)
    - [4.4 LoRA+: 改进的初始化策略](#44-lora-改进的初始化策略)
    - [4.5 VeRA: 向量随机矩阵适应](#45-vera向量随机矩阵适应)
  - [5. 其他PEFT方法](#5-其他peft方法)
    - [5.1 Adapter: 适配器层](#51-adapter适配器层)
    - [5.2 Prefix-Tuning: 前缀微调](#52-prefix-tuning前缀微调)
    - [5.3 P-Tuning v2 与 Prompt Tuning](#53-p-tuning-v2-与-prompt-tuning)
  - [6. PEFT方法选择指南](#6-peft方法选择指南)
- [三、动手实践: 微调你的第一个聊天机器人](#三动手实践微调你的第一个聊天机器人)
  - [步骤0: 环境准备](#步骤0环境准备)
  - [步骤1: 准备指令数据集](#步骤1准备指令数据集)
  - [步骤2: 配置量化与适配器](#步骤2配置量化与适配器)
  - [步骤3: 加载模型与分词器](#步骤3加载模型与分词器)
  - [步骤4: 配置训练参数](#步骤4配置训练参数)
  - [步骤5: 开始训练](#步骤5开始训练)
  - [验证微调效果](#验证微调效果)
- [四、微调深度理解](#四微调深度理解)
  - [4.1 指令数据构建的艺术](#41-指令数据构建的艺术)
    - [(1) 指令数据的黄金标准](#1-指令数据的黄金标准)
    - [(2) Self-Instruct: 用GPT-4生成训练数据](#2-self-instruct用gpt-4生成训练数据)
    - [(3) 数据增强技术](#3-数据增强技术)
  - [4.2 灾难性遗忘](#42-灾难性遗忘catastrophic-forgetting)
    - [缓解策略1: 混合训练数据](#缓解策略1混合训练数据)
    - [缓解策略2: Elastic Weight Consolidation (EWC)](#缓解策略2elastic-weight-consolidation-ewc)
  - [4.3 多任务微调](#43-多任务微调multi-task-fine-tuning)
    - [(1) 任务标识符(Task Prefix)](#1-任务标识符task-prefix)
    - [(2) 任务特定适配器](#2-任务特定适配器task-specific-adapters)
  - [4.4 持续学习](#44-持续学习continual-learning)
    - [(1) 渐进式LoRA](#1-渐进式loraprogressive-lora)
    - [(2) 知识蒸馏](#2-知识蒸馏knowledge-distillation)
    - [(3) 完整持续学习流程](#3-完整持续学习流程)
- [五、模型合并技术](#五模型合并技术model-merging)
  - [5.1 线性插值合并](#51-线性插值合并weight-averaging)
  - [5.2 SLERP: 球面线性插值](#52-slerp球面线性插值)
  - [5.3 TIES: 修剪、选举与合并](#53-tiestrim-elect-sign--merge)
  - [5.4 DARE: 丢弃与重缩放](#54-daredrop-and-rescale)
  - [5.5 Task Arithmetic: 任务算术](#55-task-arithmetic任务算术)
  - [5.6 合并方法对比与选择](#56-合并方法对比与选择)
...
```mermaid
graph LR
    A[预训练模型<br/>通用语言能力] -->|初始化| B[微调阶段]
    B -->|任务数据| C[专用模型<br/>特定任务能力]
...
#### 4.4 LoRA+: 改进的学习率策略 (2024)

**核心论文**: *LoRA+: Efficient Low Rank Adaptation of Large Models* (arXiv:2402.12354)

**核心发现**: LoRA的标准初始化存在学习率不匹配问题。

**问题分析**:

在标准LoRA中,$\Delta W = BA$:
- $B$ 初始化为0 → 初始梯度为0
- $A$ 使用Kaiming初始化 → 初始梯度较大
- 使用相同学习率会导致$B$更新太慢

**LoRA+改进**: 为矩阵A和B使用不同的学习率:
...
$$
\frac{\partial \mathcal{L}}{\partial B} = \frac{\partial \mathcal{L}}{\partial \Delta W} A^T, \quad
...
$$
\left\| \frac{\partial \mathcal{L}}{\partial A} \right\| \approx 0
$$

为了平衡梯度幅度,需要 $\eta_A \gg \eta_B$。
...
def create_lora_plus_optimizer(model, lr_base: float = 1e-4, lr_ratio: float = 16.0):
    """
    创建LoRA+优化器(A和B使用不同学习率)
    """
    # 分离LoRA A和B参数
    lora_A_params = []
    lora_B_params = []
    ...
    optimizer = torch.optim.AdamW([
        {"params": lora_B_params, "lr": lr_base},
        {"params": lora_A_params, "lr": lr_base * lr_ratio},
        {"params": other_params, "lr": lr_base},
    ])

    print(f"LoRA+优化器配置:")
    print(f"  LoRA B学习率: {lr_base}")
    print(f"  LoRA A学习率: {lr_base * lr_ratio} (x{lr_ratio})")
    print(f"  其他参数学习率: {lr_base}")
...
optimizer = create_lora_plus_optimizer(model, lr_base=1e-4, lr_ratio=16.0)
```
...
with open("train_data.jsonl", "w", encoding="utf-8") as f:
    for sample in train_data:
        f.write(json.dumps({
            "messages": sample.to_conversation_format()
        }, ensure_ascii=False) + "\n")

print(f"已保存 {len(train_data)} 条训练样本")
```

**数据预处理：Tokenization (支持Chat Template)**

Hugging Face的`apply_chat_template`是处理对话数据的最佳实践，它会自动处理特殊token（如EOS, BOS, role markers）。
...
class DatasetProcessor:
    ...
    def tokenize_function(self, examples: Dict[str, List]) -> Dict[str, torch.Tensor]:
        """
        利用apply_chat_template处理对话数据
        """
        input_ids_list = []
        labels_list = []

        for messages in examples["messages"]:
            # 1. 使用Chat Template格式化整个对话
            # result example: "<s>[INST] instruction [/INST] output </s>"
            ...
            # 2. 掩码处理：只计算assistant回复部分的loss
            # 分离prompt和response来确定掩盖范围
            user_messages = messages[:-1] # 除去最后一条assistant回复
            prompt_text = self.tokenizer.apply_chat_template(
                user_messages,
                tokenize=False,
                add_generation_prompt=True # 添加生成引导符
            )
        ...
        # 3. 手动Padding (确保batch内对齐)
        # 注意：这里需要在DataCollator中处理padding，或者在这里做
        # 为简单起见，这里假设使用DataCollatorForSeq2Seq或自定义padding
...
# 处理数据 (注意：transformers tokenizer输出不仅是ids，这里简化处理用于演示)
# 实际训练中通常在DataCollator中做padding
...
#### 4.5 显存优化：Gradient Checkpointing

**核心原理**：一种**"用时间换空间"**的技术。

在标准的深度神经网络训练中，前向传播（Forward Pass）时计算的所有中间激活值（Activations）都需要保存在显存中，以便在反向传播（Backward Pass）时计算梯度。

- **标准训练**：存储 O(N) 层激活值。显存占用随层数线性增加。
- **Gradient Checkpointing**：
  1. **前向传播**：只存储部分关键节点的激活值，丢弃其他中间结果。
  2. **反向传播**：当需要用到被丢弃的激活值计算梯度时，利用存储的关键节点**重新计算**前向传播。

**权衡分析**：
- **显存节省**：可减少 50%-80% 的激活值显存占用（通常激活值是显存大户）。
- **计算代价**：训练时间增加约 20%-30%（因为部分前向传播需要计算两次）。
...
假设网络有 N 层，标准反向传播需要存储所有 N 层的输出。
Gradient Checkpointing 可以将网络划分为 $\sqrt{N}$ 个段，只存储每段的输入。

- 显存复杂度：$O(N) \to O(\sqrt{N})$
- 示例：对于 32 层的 LLaMA 模型，原本需存 32 层激活，现在只需存 $\sim 6$ 个检查点。

**实战建议**：
1. **何时开启**：
   - 只要显存不足（OOM），这就是第一个应该开启的选项。
   - 批次大小（Batch Size）无法调大时。
   - 使用长序列训练时（激活值与序列长度成正比）。

2. **注意事项**：
   - 如果显存充足，不要开启，以换取更快的训练速度。
   - 在 QLoRA 中，通常默认建议开启，因为显存是大语言模型微调的主要瓶颈。
...
│
├── 工程实践(第三节)
│   ├── 环境搭建(Unsloth加速框架成为主流)
│   ├── 数据准备(Instruction-Output格式 + Chat Template)
│   ├── 训练流程(SFTTrainer + Flash Attention 2)
│   └── 效果验证(LLM-as-a-Judge)
│
├── 深度理解(第四节)
│   ├── 指令数据构建
│   │   ├── Self-Instruct (GPT-4合成)
│   │   └── Evol-Instruct (进化指令)
│   ├── 灾难性遗忘
│   │   ├── 混合训练数据(Replay)
│   │   └── EWC正则化
...
**LoRA家族性能对比** (LLaMA-3-8B, GSM8K数学推理任务):

| 方法 | 可训练参数 | 显存占用 | 准确率 | 训练时间 |
|-----|-----------|---------|-------|---------|
| 全量微调 | 8.0B (100%) | 96GB | 72.3% | 48小时 |
| LoRA r=16 | 8.4M (0.1%) | 18GB | 71.8% | 12小时 |
| QLoRA r=16 | 8.4M (0.1%) | 6.0GB | 71.5% | 14小时 |
| DoRA r=16 | 8.4M (0.1%) | 18GB | 73.1% | 15小时 |
| LoRA+ r=16 | 8.4M (0.1%) | 18GB | 72.5% | 12.5小时 |
...
- DPO：无需RL的偏好对齐 (Direct Preference Optimization)
- ORPO、KTO等最新方法 (2024-2025)
- 如何构建偏好数据集 (Chosen vs Rejected)
...
现在，你可以：
1. 用16GB显存微调70B模型 (使用QLoRA)
2. 构建持续学习系统 (使用EWC或LoRA)
3. 合并多个专家模型 (使用Task Arithmetic)
4. 避免90%的常见陷阱
